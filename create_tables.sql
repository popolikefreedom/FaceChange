...
CREATE OR REPLACE FUNCTION cacuclate_enmy_gold(value INTEGER, match_ids BIGINT, steam_id BIGINT, indexs INTEGER) RETURNS integer AS $$
    DECLARE
    ...
    BEGIN
      SELECT position INTO player_position from players_pos WHERE players_pos.steamId=steam_id;
      IF player_position=2 THEN
        SELECT gold_t[7] as gold, lh_t[7] as lh, xp_t[7] as xp INTO enemy FROM player_matches WHERE player_matches.match_id=match_ids AND player_matches.account_id <> steam_id AND player_matches.account_id IN (SELECT steamid from players_pos WHERE players_pos.position=2);
